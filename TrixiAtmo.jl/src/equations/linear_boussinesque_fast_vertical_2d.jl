...
@muladd begin
    ...
    """
    struct LinearizedGravityWaveEquationsFastVertical2D{RealT <: Real} <:
    ...
    function LinearizedGravityWaveEquationsFastVertical2D(cs::Real, U::Real, fb::Real)
        return LinearizedGravityWaveEquationsFastVertical2D(cs, U, fb)
    end
    
    function varnames(::typeof(cons2cons), ::LinearizedGravityWaveEquationsFastVertical2D)
        ("u", "w", "p", "b")
    end
    function varnames(::typeof(cons2prim), ::LinearizedGravityWaveEquationsFastVertical2D)
        ("u", "w", "p", "b")
    end
   ...
   # u_normal =  normal[2] * u_inner[2]
    ...
    @inline function rotate_to_x(u, normal_vector, equations::LinearizedGravityWaveEquationsFastVertical2D)
    ...
    @inline function flux(u, normal_direction::AbstractVector,
        ...
        v_normal = v2 * normal_direction[2]
    ...
    @inline have_constant_speed(::LinearizedGravityWaveEquationsFastVertical2D) = True()
    
    @inline function max_abs_speeds(equations::LinearizedGravityWaveEquationsFastVertical2D)
        @unpack cs = equations
        return cs, cs
    end
    ...
    @inline function flux_lmars(u_ll, u_rr, orientation::Integer,
        ...
        p2_int = (p_ll + p_rr) * 0.5 
        v1_int = (v1_ll + v1_rr) * 0.5 - 1 / (2 * cs) * (p_rr - p_ll)
        v2_int = (v2_ll + v2_rr) * 0.5
        if orientation == 1
            f1 = 0.0  
            f2 = 0.0
            f3 = 0.0
    ...
    @inline function flux_lmars(u_ll, u_rr, normal_direction::AbstractVector,
        ...
        v_ll =   v2_ll * normal_direction[2]
        v_rr =   v2_rr * normal_direction[2]
        p_interface = 0.5 * (p_ll + p_rr)
        v_interface = 0.5 * (v_ll + v_rr)
    ...
    @inline cons2prim(u, equations::LinearizedGravityWaveEquationsFastVertical2D) = u
    @inline cons2entropy(u, ::LinearizedGravityWaveEquationsFastVertical2D) = u
    end # muladd
